...
namespace propelcog\craftorderimport\elements;
...
class CommerceOrder extends Element
{
   ...
   # use OrderNoticesTrait;
    ...
    public  $orderLastFour = null;
    public  $orderTransactionId = null;
    public  $totalDiscount = null;
    public  $orderVaultId = null;
    public  $shippingMethodAmount = null;
    public  $lineItems = null;

    public  $taxMethodName = null;
    ...
    */
    ...
    public function adjustment(array $adjustment): void
    {
        ...
        $record->type = $adjustment['type'];
        $record->description = $adjustment['description'];
        $record->amount = $adjustment['amount'];
        $record->lineItemId = $adjustment['lineItemId'];
        $record->orderId = $adjustment['orderId'] ;
        $record->isEstimated = $adjustment['isEstimated'];
        $record->sourceSnapshot = $orderRecord->toArray();
        $record->save(false);
    ...
    {
        $order = Commerce::getInstance()->getOrders()->getOrderById($this->id);
        ...
        if( !empty( $mfields ) && !empty( $order ))
        {
            foreach( $mfields as $key => $val)
            {
               ...
               //  print_r( $fldData->$val );
               ///  echo "\n";
               // $orderRecord->setFieldValue( $fldData->$val ) ;
              //  echo $orderRecord->$val;
               // echo "\n";
    ...
    */
    public function beforeSave(bool $isNew): bool
    {
       // echo"<pre>";print_r($this->lineItems);die;
        ...
        foreach($this->lineItems as $key => $item)
        {
            ...
            $pid = $lastElementInserID;
            $purchasable                = new PurchasableRecord();
            $purchasable->sku           = $item['lineItemssku'];
            $purchasable->price         = $item['lineItemsprice'];
            $purchasable->description   =  $desc;
            $purchasable->id            =  $pid;
            $purchasable->save(false);

            $uidi = StringHelper::UUID();
            $uids = StringHelper::UUID();
            $results = \Craft::$app->db->createCommand()->insert('commerce_lineitems', [
                'orderId'               => $this->id,
                'purchasableId'         => $pid,
                'taxCategoryId'         => $item['lineItemstaxCategoryId'],
                'shippingCategoryId'    => $item['lineItemsshippingCategoryId'],
                'description'           => $desc,
                'price'                 => $item['lineItemsprice'],
                'saleAmount'            => $item['lineItemssaleAmount'],
                'salePrice'             => $item['lineItemssalePrice'],
                'sku'                   => $item['lineItemssku'],
                'subtotal'              => $item['lineItemssubtotal'],
                'total'                 => $item['lineItemstotal'],
                'qty'                   => $item['lineItemsqty'],
                'options'               => $item['lineItemsoptions'],
                'optionsSignature'      => $uidi,
                'uid'                   => $uids,
                'snapshot'              => json_encode(array(time())),
                'note'                  => $item['lineItemsnote'],
                'privateNote'           => $item['lineItemsprivateNote'],
    ...
    public function afterSave(bool $isNew): void
    {
       ...
       // $orderRecord->isCompleted = $this->isCompleted;
        ...
        $orderRecord->shippingMethodHandle = str_replace(' ','',strtolower($this->shippingMethodName)) ?? '';
       ...
       // $orderRecord->recalculationMode = $this->getRecalculationMode();
        ...
        if( $this->totalDiscount && $this->couponCode ){
            ...
            $record->description = $this->couponCode;
            $record->orderCondition = ["elementType"=>null,"fieldContext"=>"global","class"=>"craft\\commerce\\elements\\conditions\\orders\\DiscountOrderCondition","conditionRules"=>[]];
            $record->customerCondition = ["elementType"=>null,"fieldContext"=>"global","class"=>"craft\\commerce\\elements\\conditions\\customers\\DiscountCustomerCondition","conditionRules"=>[]];
            $record->shippingAddressCondition = ["elementType"=>null,"fieldContext"=>"global","class"=>"craft\\commerce\\elements\\conditions\\addresses\\DiscountAddressCondition","conditionRules"=>[]];
            $record->billingAddressCondition = ["elementType"=>null,"fieldContext"=>"global","class"=>"craft\\commerce\\elements\\conditions\\addresses\\DiscountAddressCondition","conditionRules"=>[]];
            $record->baseDiscount = $this->totalDiscount;
            $record->baseDiscountType = 'value';
            $record->enabled = 1;
            $record->save(false);

            if( $this->couponCode ){
                $couponCodeMethod = (new Query())->select(['code','id'])->from(['commerce_coupons'])->where(['code' => $this->couponCode])->one();
                if( !$couponCodeMethod ){
                    $record2 = new CouponRecord();
                    $record2->code = $this->couponCode;
                    $record2->discountId = $record->id;
                    $record2->save(false);
                }
                $orderRecord->couponCode = $this->couponCode;
        ...
        if( $this->shippingMethodName && $this->shippingMethodAmount ){
            $shippingHandle = str_replace(' ','',strtolower($this->shippingMethodName));
            $shippingMethod = (new Query())->select(['name','handle','id'])->from(['commerce_shippingmethods'])->where(['handle' => $shippingHandle])->one();
            if( !$shippingMethod ){
                $liteMethod = new ShippingMethodRecord();
                $liteMethod->isLite = true;
                $liteMethod->name = $this->shippingMethodName;
                $liteMethod->handle = $shippingHandle;
        ...
        if( $this->taxMethodName && $this->totalTax ){
            $taxHandle = str_replace(' ','',strtolower($this->taxMethodName));
            $taxMethod = (new Query())->select(['name','handle','id'])->from(['commerce_taxcategories'])->where(['handle' => $taxHandle])->one();
            if( !$taxMethod ){
                $liteMethod = new TaxCategoryRecord();
                $liteMethod->name = $this->taxMethodName;
                $liteMethod->handle = $taxHandle;
        ...
        $orderTransactionId = ( $this->orderTransactionId )? $this->orderTransactionId :'';
        $orderVaultId = ( $this->orderVaultId )? $this->orderVaultId :'';

        $orderLastFour = ( $this->orderLastFour )? $this->orderLastFour :'';
        $transaction = new TransactionRecord();
       // $transaction->status = TransactionRecord::STATUS_PENDING;
       $transaction->status = TransactionRecord::STATUS_SUCCESS;
        if (Commerce::getInstance()) {
            $paymentCurrency =  Commerce::getInstance()->getPaymentCurrencies()->getPaymentCurrencyByIso( $this->paymentCurrency );
            $currency = Commerce::getInstance()->getPaymentCurrencies()->getPaymentCurrencyByIso( $this->currency );
            $transaction->gatewayId = $this->gatewayId;
            $transaction->currency = $currency->iso;
            $transaction->paymentCurrency = $paymentCurrency->iso;
            $transaction->paymentAmount = Currency::round($this->getTotal(), $paymentCurrency);
           // $transaction->paymentAmount = $orderRecord->total;

          $amount = Commerce::getInstance()->getPaymentCurrencies()->convertCurrency($transaction->paymentAmount, $transaction->paymentCurrency, $transaction->currency);
         // $transaction->amount = Currency::round($amount, $currency);
          ...
          $paymentSourceRecord->gatewayId = $this->gatewayId;
          // $paymentSourceRecord->token = $orderTransactionId;
        ...
        // $orderRecord->totalPaid = $this->getTotalPaid();
         $orderRecord->totalPrice = $this->getTotal();
         $orderRecord->totalPaid = $this->getTotal();
    ...
    public function random_order_id($eid){
        $orderRecord = new OrderRecord();
        $orderRecord->id = $eid;
        $orderRecord->orderLanguage = 'en';
        $orderRecord->save(false);
    }
    public function getCustomCustomerID(?string $email,?string $firstName): ?int
    {
        $customerID = 273350;
        $users = \craft\elements\User::find()->email($email)->one();
        if( $users ){
            $customerID = $users->id;
        }else{
            $password = "test@123";
    ...
    public function getCustomSetBillingAddress(AddressElement|array|null $address,?int $customerID,?int $elementID): ?int
    {
        ...
        ],'id ='.$elementID)->execute();
        $address->title = Craft::t('commerce', 'Billing Address');
        $users = \craft\elements\User::find()->id($customerID)->one();
        Plugin::getInstance()->getCustomers()->savePrimaryBillingAddressId($users, $addressElement->id);
        return $address->id;
    }

    public function getCustomSetShippingAddress(AddressElement|array|null $address,?int $customerID,?int $elementID): ?int
    {
        $addressElement = new AddressElement();
        $addressElement->setAttributes($address);
        $addressElement->ownerId = $customerID;
